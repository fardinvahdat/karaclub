<template>
  <div>
    ...
    <div class="main-navbar">
      <div id="mainnav">
        <div class="toggle-nav p-0" :class="leftSidebarVal ? 'toggle-button' : ''" @click="openmobilenav=true">
          <i class="fa fa-bars sidebar-bar"></i>
        </div>
        <ul class="nav-menu" :class="{ opennav: openmobilenav }" >
          <li class="back-btn">
            <div class="mobile-back text-right">
              <i class="fa fa-angle-right pl-2 position-static" aria-hidden="true"></i>
          ...
          <li v-for="(menuItem, index) in menulist" :key="index" :class="menuItem.megamenu ? 'mega-menu' : 'dropdown'">
            <i :class="menuItem.icon" class="d-inline d-xl-none"></i>
            <nuxt-link :to="menuItem.path" class="nav-link" @click="setActive(menuItem.title)">
              {{menuItem.title}}
