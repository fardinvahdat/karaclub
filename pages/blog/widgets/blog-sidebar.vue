<template>
      ...
      <div class="blog-sidebar">
        <div class="theme-card">
          <h4 class="text-right">مقالات اخیر</h4>
          <ul class="recent-blog text-right px-0">
            <li v-for="(blog,index) in bloglist.slice(0, 6)" :key="index">
              <div class="media">
                <img :src="getImgUrl(blog.images[0])" class="img-fluid" alt />
                <div class="media-body align-self-center mr-1">
                  <h6>{{ blog.date }}</h6>
                  <p>{{ blog.title }}</p>
                </div>
              </div>
        ...
        <div class="theme-card">
          <h4 class="text-right">تگ های پرطرفدار</h4>
          <ul class="popular-tag text-right px-0 d-flex">
